...
class HomeController extends Controller
{
    ...
    public function reactionStore(Request $request)
    {
        ...
        try {
            ...
            $isReacted = PostReaction::where('user_id',Auth::id())->where('post_id',$attribute['post_id'])->exists();
            if($isReacted)
                return response()->json(['message' => 'Reaction already given']); 
    ...
    public function reactionDelete(Request $request)
    {
         ...
         $reaction = PostReaction::where('user_id', Auth::id())->where('post_id', $request->post_id);
        $reaction->delete();
